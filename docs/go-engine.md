...
Provider/Checker 由 `namespace:name:version` 标识。`LXP_CONFIG` 可指向本地 EngineConfig；默认读取用户配置目录的 `lxp/config.yaml`，未找到时使用官方 builtin binding。配置可以选择 `source: helper` 的 argv，或从显式授权的 OCI repository 按 digest 安装；Helper 逐行 JSON 握手并在当前命令结束时退出。仓库、command、implementation 版本和 digest 不进入 Artifact。详见[扩展协议](extensions.md)。
