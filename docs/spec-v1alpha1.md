...
EngineConfig 中 `repositories` 是本地有序候选仓库列表，`bindings` 把 Provider/Checker contract 映射到精确 implementation package 坐标。Repository ID MUST 唯一；每个 contract MUST 只有一个 binding，并不得跨 Provider/Checker kind 复用。`source: repository` 还 MUST 固定 SHA-256：当前仓库没有该坐标时继续下一个；一旦命中坐标却 digest 不符，MUST 立即失败，不得继续搜索或降级；首个验证通过的结果胜出。仓库 credential 只能引用本地 secret slot，值不得写入配置。Artifact 不得覆盖仓库顺序、镜像、trust 或安装 policy。配置案例见 [engine-config.yaml](../examples/config/engine-config.yaml)。

本地注册的 Provider/Checker MUST 声明自己的 implementation package 坐标。Engine 在新的或 `importing` 状态的 Import，以及 Add、Status、Export 前 MUST 精确比较注册实现与 binding；不得因为 contract 相同就使用另一个 package/version。Import 首次进入 `importing` 时 MUST 把解析结果固定在本地 Session state；重试配置发生变化时必须失败，不得换实现继续 Apply。相同 digest 的 `ready` Session 在安全验证 envelope 与本地身份后直接 no-op，不再解析扩展、运行 Checker 或调用 Provider。该 state 不是 Artifact 内容。

自动安装不是 v1alpha1 要求，v1alpha1 也不定义可互操作的 package layout、ABI 或进程协议。实现 MAY 根据操作者配置下载并验证自有格式的包，但 MUST NOT 因 Artifact 单独触发安装或执行。官方 Production MVP 只执行显式编入 CLI 的 Go `builtin` 实现；LXP 不承诺维护多语言 SDK。
...
2. 根据本地 EngineConfig 精确解析全部 Provider 与 Checker contract；
...
某 Component 失败时 Import 返回错误并保留 target、已完成 observed state 与 marker，以便相同命令重试。重试 target 中除 `.lxp`、desired Component root 及其祖先外出现无归属内容 MUST 失败。Core MUST NOT 回滚已成功 Component，也不得删除整个 target。局部临时文件与 owning Provider 的部分 root 可由各自实现清理。Import 不包含 install、build 或 Activate 阶段。
