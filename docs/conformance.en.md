...
| `production-git-v1` | official CLI composes `git@v1` only; supports reference/embedded/mirrored; preserves index selection, minimal HEAD bundles, and parent advancement | embedded standalone round trip; online reference; offline reference failure/cleanup; offline mirrored fallback; rejection of secret/local locators, revision mismatch, invalid staged index, shallow repositories, submodules, escaping symlinks, and extra payload roles; LFS pointer mode never executes filters |
