...
| `tracking-core` | Init/Import, longest-root Add/Status routing, nested ownership exclusions, parent-to-child Import, child-to-parent Export, and parent provenance | black-box CLI/API tests; nested discovery, child routing, symlink/non-empty collisions, and unowned paths blocking Export |
...
| `production-git-v1` | official CLI composes `git@v1` only; supports reference/embedded/mirrored, recursive submodule Components, index selection, minimal HEAD bundles, and parent advancement | all three submodule distributions; offline embedded/mirrored child restore; strict gitlink/child-revision validation; rejection of uninitialized/unregistered submodules, secret/local locators, invalid staged indexes, shallow repositories, escaping symlinks, and extra payload roles; LFS pointer mode never executes filters |
