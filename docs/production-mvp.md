...
Import 在任何 Component 写入前验证完整 Artifact、按本地 policy 解析全部扩展、检查 consumed Requirements，并调用所有 Provider `Validate`；随后父到子 `Apply`。Helper 下载与握手属于 extension resolution；没有 Artifact 驱动的 install/build、独立 Activate、Plan command 或 Core rollback 阶段。
...
- 官方 CLI 默认使用 builtin Git Provider，另提供独立 `lxp-provider-git` Helper 作为首个标准扩展样例。Engine 支持本地 `helper` argv，以及从显式 `auto_install`、namespace allowlist 的 OCI repository 按 digest 下载 Helper；Artifact 永远不能覆盖仓库、镜像、trust、command 或安装 policy。
...
本 Profile 不提供中心化 LXP Registry 服务、全局插件搜索、Artifact 驱动安装、in-process 插件 ABI、Template import、branch/merge/rebase、多父历史、multi-writer、远端 revision 自动前进、外部 Git LFS object exchange、execution replay 或 hostile Artifact/Helper sandbox。
