...
Ordered `repositories` in EngineConfig are local OCI candidates. `bindings` map Provider/Checker contracts to exact implementation package coordinates. Repository IDs MUST be unique; each contract has exactly one binding and cannot be reused across Provider/Checker kinds. A binding selects `builtin`, local `helper` argv, or `repository`. The last MUST pin an OCI manifest SHA-256 and can download only from a repository with `auto_install: true` that explicitly trusts the implementation namespace. An absent digest continues to the next repository; after content is obtained, any digest, descriptor, platform, or handshake mismatch MUST fail without further search or fallback. Repository credentials can only refer to local secret handles; values cannot appear in config. An Artifact cannot override repository order, mirrors, trust, or installation policy. See the [Extension and Helper protocol](extensions.en.md) and [engine-config.yaml](../examples/config/engine-config.yaml).

Every local Provider or Checker, whether builtin or Helper, MUST declare its implementation package coordinate. Before a new or `importing` Import, Add, Status, or Export, Engine MUST compare the registration/handshake result exactly with the binding; a matching contract cannot authorize a different package or version. When an Import first enters `importing`, its resolved implementations MUST be pinned in local Session state. A retry fails if configuration would change them instead of continuing with another implementation. A same-digest `ready` Session returns no-op after safe envelope and local-identity validation, without resolving extensions, running Checkers, or calling Providers. This state is not Artifact content.

`v1alpha1` defines a language-neutral Helper process protocol and platform-specific OCI Extension Package, but no in-process ABI and no multi-language SDK promise. An Engine MAY download a Helper by digest only when local policy explicitly enables it; it MUST execute argv directly, verify the handshake exactly, propagate deadlines, bound diagnostics, and close the process at command end. An Artifact contract alone can never authorize installation or execution. The default official composition MAY retain a builtin Git Provider to avoid a network dependency on first use while still allowing a local EngineConfig to select an independent Helper.
...
2. resolve every Provider and Checker contract exactly through local EngineConfig, installing by digest from an authorized OCI repository and handshaking a Helper when needed;
...
When a Component fails, Import returns the error and preserves the target, completed observed state, and marker for the same command to retry. A retry MUST fail if the target contains unowned content outside `.lxp`, desired Component roots, and their ancestors. Core MUST NOT roll back successful Components or delete the target. Local temporary files and Provider-owned partial roots may be cleaned by their owners. Helper installation and handshake are local extension resolution; Import has no Artifact-driven install, build, or separate Activate stage.
