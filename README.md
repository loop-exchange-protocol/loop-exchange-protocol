...
LXP 不是部署模板、包安装器或工作流语言。YAML 是机器交换格式，日常入口是 CLI；Production MVP Artifact 内容寻址且不可变，其中 embedded 提供 standalone 恢复。
...
1. **LXP 管 ownership，Provider 管内容。** Component root 除 direct child boundaries 外对 Core 不透明。
2. Component roots 必须唯一，可以形成严格嵌套的 lexical tree；每个实体 path 只归最深 root，祖先 Provider 排除 child subtree。
3. `lxp add` 默认调用最深 owning Provider；Provider 的 native discovery 可把已初始化 submodule 等明确 child root 注册为嵌套 Component。
4. Provider 由稳定的 `provider + contract` 标识；Provider-specific `config` 对 Core 不透明。
5. 协议不定义 mount capability 或 Provider 组合矩阵；具体父子路径物理不兼容时 Provider 必须 fail closed。
...
8. Production MVP 只装配 `git@v1`，但完整支持 reference、embedded 与 mirrored `.lxpz`；无匹配 Provider 时失败。

公开 CLI 使用 `lxp export --distribution reference|embedded|mirrored` 选择形式（默认 embedded），Import 按 Artifact 声明自动处理。选择规则见 [Distribution 指南](docs/distributions.md)。
...
Git Component 内的 `lxp add` 调用 Git Provider 的原生 index 语义；已初始化 submodule 自动成为嵌套 Git Component，父 gitlink 与子 locked revision 在 Export 时强校验。未归属 path 没有匹配 Provider 时失败。`lxp import` 会在任何副作用前验证 Artifact、展示 Provider actions 并检查 Requirements。
...
- [Distribution 指南](docs/distributions.md) · [English](docs/distributions.en.md)
...
- [Reference/Mirrored YAML 示例](examples/distributions/README.md) · [English](examples/distributions/README.en.md)
- [嵌套 Component/Submodule YAML 示例](examples/submodules/README.md) · [English](examples/submodules/README.en.md)
